...
| `{obs_id}_*_gain_outliers_tiles.png` | Plot of the amplitude-outlier gains that were detected and removed (Step 4) and any channels cut off by the Step 3 gain-magnitude sanity check, shown against the fitted curve and acceptance band, per tile. The two are visually distinguished: amplitude outliers get a red shaded band, a black 'x' marker, and a red tile border; gain-cutoff divergences get an orange shaded band, a black '+' marker, and an orange tile border (orange takes precedence for a partially-flagged tile with both). A tile flagged before Calvin's own analysis ran (metafits/TILES-HDU/BASELINES-HDU) has no real data to show, so it gets a red "flagged in ..." message, top-centre, on an otherwise blank panel with a red border. A tile Calvin itself fully flagged (e.g. promoted via Step 5) still has real data, so it's plotted normally with the same red message and border overlaid on top, rather than being hidden behind a placeholder. |
