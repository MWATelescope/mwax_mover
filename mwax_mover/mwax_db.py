...
def get_data_file_row(db_handler_object, full_filename: str) -> DataFileRow:
    ...
    try:
        ...
        obsid, year, month, day, size, checksum = db_handler_object.select_one_row(
            sql, (filename,)
        )
...
def upsert_data_file_row(
    ...
    try:
        if checksum_type is None:
            ...
            db_handler_object.upsert_one_row(
                sql,
                (
                    str(obsid),
                    filetype,
        ...
        if db_handler_object.dummy:
            ...
            with db_handler_object.db_lock:
                db_handler_object.logger.warning(
                    f"{filename} upsert_data_file_row() Using dummy database connection. "
        ...
        db_handler_object.logger.error(
            f"{filename} insert_data_file_row() error upserting data_files record in "
            f"data_files table: {upsert_exception}. SQL was {sql}"
        )
        return False
