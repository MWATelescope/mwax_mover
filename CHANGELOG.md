...
* Calvin:
  * Change reftile good dipole gate to be 32/32 instead of 30/32.
...
* Replaced `HyperfitsSolutionGroup.refant` (picked the lowest-ID unflagged tile, with no calibration-quality signal at all) with `select_refant(phase_fit_niter)`, a two-stage bootstrap-then-rank selection. Stage 1 runs a throwaway, read-only phase/gain-fit pass — using a cheap structural bootstrap tile (the old lowest-ID logic, kept as private `_bootstrap_refant()`) purely to gather per-tile ranking data, never mutating the in-memory solutions. Stage 2 scores every unflagged tile by how many quality gates it fails (phase-fit quality ≥ 0.8, phase-fit chi2dof in `[0.2, 3.0]`, gain-fit quality ≥ 0.8 — each checked on the worse of XX/YY, so a tile is only as trustworthy as its worse polarisation) and, among tiles with equal failure counts, by how far its fitted phase-slope length deviates from the *population median* length rather than from zero — median-relative deviation is invariant to which tile the bootstrap stage happened to use as its reference, unlike the raw fitted value, which trivially reads 0 for whichever tile was used to compute it. Tile ID breaks any remaining tie, for a deterministic result. Degrades gracefully when no tile passes every gate: the tile failing fewest still wins, with no separate "nothing qualified" case needed. New constants: `REFTILE_PHASE_QUALITY_MIN`, `REFTILE_PHASE_CHI2DOF_MIN`/`MAX`, `REFTILE_GAIN_QUALITY_MIN`. New `df_columns` entries: `COL_LENGTH`, `COL_QUALITY` (promoted from raw string literals once `select_refant` became a second consumer). Full design/rationale in `docs/REF_TILE_SELECTION.md`.
* `calvin/plots/hyperdrive.py`'s `generate_plots`/`generate_plots_for_files` gain a `ref_tile: int | None = None` parameter, passed through as `--ref-tile {ref_tile}` on hyperdrive's own `solutions-plot` command line when given, so hyperdrive's before/after amp/phase plots use the same reference tile as calvin's internal calculations rather than hyperdrive's own default. `None` (the default) omits the flag, so existing callers are unaffected. The value is the 0-based antenna index (`mwalib.Antenna.ant`), not the tile name — hyperdrive's `--ref-tile` flag takes an antenna index, not a name. To make this available, `calibration/models.Tile` gains an `ant: int` field (replacing the former commented-out `# index: int`), populated from `Antenna.ant` in `Metafits.tiles`. `calvin/pipeline.py` and `cli/cal_utils.py` now pass `select_refant()`'s chosen tile's `ant` value through to both the "before" and "after" plot calls — no reordering was needed beyond what the ref-tile-selection change above already required, since the reference tile was already selected before the "before" plots ran (needed there for `run_flagging_pipeline` regardless). Fixes two pre-existing bugs in the original wiring: the flag name was `--reftile` (hyperdrive's actual flag is `--ref-tile`), and the value passed was the tile name string rather than the antenna index.
* For Calvin plots that use a ref-tile, the ref tile selected is clearly annotated on the plot's subtitle.

## mypy cleanup

* Added `mypy` to the project's dev tooling, alongside the existing `ruff`/`ty`. Fixed all 43 pre-existing errors it found across 16 files (unrelated to the reference-tile work above) — every one traced back to one of: a redundant type re-annotation on a variable already typed by an earlier, unannotated assignment; a variable typed too narrowly from whichever branch/assignment mypy saw first (e.g. `str` vs `list[str]` depending on `shell=`; `ArchivePriority` vs `int`, though `ArchivePriority` *is* an `int` via `IntEnum`); a heterogeneous list/dict inferred too narrowly from its first entry; a custom validator (`is_int()`) that doesn't narrow a type the way `isinstance()` does; a genuine stub/annotation imprecision (`fit_phase_line`'s `freqs_hz` rebinding from `NDArray[np.float64]` to an astropy `Quantity` partway through the function, now renamed to `freqs_hz_qty` from that point on); or a deliberate, already-documented Python idiom (`__hash__ = None` to suppress hashability) that mypy's stub doesn't model. No behaviour changes — every fix is a rename or an annotation, verified against each touched file's existing test coverage where one exists.

## Phase-fit iterative refit bug fix

* Fixed `fit_phase_line`'s `niter > 1` iterative sigma-clip refit, which never actually re-narrowed the frequency array used in the fit on later iterations — a leftover, never-read `freqs_hz = freqs_hz[mask]` line updated the wrong (dead, pre-unit-conversion) variable instead of `freqs_hz_qty`, the one actually passed to `minimize()`/`model()`. Any iteration that clipped even one channel caused the *next* iteration to fit mismatched-length frequency and solution arrays and raise `ValueError`. That exception is caught by `HyperfitsSolutionGroup._phase_fit_one` and logged only as a warning, silently dropping that tile/pol's phase fit from `process_phase_fits`'s output rather than crashing visibly. **Production runs `phase_fit_niter=3`**, and this fired whenever sigma-clipping rejected any channel before the last iteration — the normal case for real RFI-contaminated data, not an edge case — so the iterative-refit feature has likely not been functioning for any tile that actually needed it (i.e. had a genuine outlier channel), which is exactly the case it exists to handle. One-line fix (`freqs_hz_qty = freqs_hz_qty[mask]`); new regression test covers a realistic contamination scenario (a handful of outlier channels plus modest phase noise) that neither existing `niter` test covered — one used a perfectly clean ramp where nothing is ever clipped, the other used 95% contamination that exits the loop before a second iteration is ever reached.
* Fixed a stale test (`test_estimate_di_calibrate_peak_ram_bytes_matches_manual_calculation`): its manual mirror calculation still used the old `n_shapelets = num_sources // 8` guess, but the real function was deliberately changed to hardcode `n_shapelets = 0` in an earlier commit ("no shapelets in the current skymodels"), and the test was never updated to match. Pre-existing staleness, unrelated to anything else in this changelog entry.

## Post Review cleanup

* Fixed stale `--reftile` references in `CALVIN.md`, `README.md`, and `docs/REF_TILE_SELECTION.md` (now `--ref-tile` everywhere, matching the code). Updated `REF_TILE_SELECTION.md`'s status from "plan only" to "implemented" and rewrote Phase 3 to reflect the actual `ref_tile: int` implementation.
* Replaced all 10 dangling `docs/HYPERDRIVE_PARALLELISM.md` references (the plan doc was never committed) with pointers to the relevant code or "during the source_code_restructure".
* Broadened `README.md`'s `constants.py` description to mention calibration numerics, daemon behaviour, and INI config constants.
* Removed dead code: `Input` namedtuple and `Metafits.inputs`/`inputs_df` (unused), `HyperfitsSolutionGroup.get_solns()` (superseded by `get_solns_both`), `HyperfitsSolution.get_average_times()`/`get_ref_solutions()`, `get_filterbank_key_value_int()`/`set_filterbank_key_value_int()`, and `all_scans_completed()` from both watch-queue-worker classes.
* Removed commented-out `median_thickness` field (3 sites), commented-out `get_length` method, debug `print()` calls in `fit_phase_line`, and `# index: int` from `Input`.
* Added `COL_NAME`, `COL_ID`, `COL_FLAG`, `COL_ANT`, `COL_RX`, `COL_SLOT` to `calibration/df_columns.py` and replaced all raw-string tile-column accesses across `hyperfits_solution_group.py`, `plots/gains.py`, `plots/stats_table.py`, and `plots/phases.py`, matching the pattern already used for fit-result columns.
