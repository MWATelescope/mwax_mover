...
* `calvin/plots/hyperdrive.py`'s `generate_plots`/`generate_plots_for_files` gain a `reftile: str | None = None` parameter, passed through as `--reftile {reftile}` on hyperdrive's own `solutions-plot` command line when given, so hyperdrive's before/after amp/phase plots use the same reference tile as calvin's internal calculations rather than hyperdrive's own default. `None` (the default) omits the flag, so existing callers are unaffected. `calvin/pipeline.py` and `cli/cal_utils.py` now pass `select_refant()`'s chosen tile through to both the "before" and "after" plot calls — no reordering was needed beyond what the ref-tile-selection change above already required, since the reference tile was already selected before the "before" plots ran (needed there for `run_flagging_pipeline` regardless).
