...
def test_correlator_config_file():
    ...
    # Setup all the paths
    base_dir = setup_test_directories("vcs_subfile")
    ...
    sd.initialise(render_test_config("vcs_subfile"), fake_db_handler)
    ...
    assert sd.cfg_corr_mwax_stats_binary_dir == get_test_bin_dir("vcs_subfile")
...
def test_process_vcs_subfile():
    ...
    # Call to read config <-- this is what we're testing!
    sd.initialise(render_test_config("vcs_subfile"))
    # Override db_handler with a fake one
