...
def test_correlator_config_file():
    ...
    # Setup all the paths
    base_dir = setup_test_directories("correlator_subfile")
    ...
    sd.initialise(render_test_config("correlator_subfile"), fake_db_handler)
    ...
    assert sd.cfg_corr_mwax_stats_binary_dir == get_test_bin_dir("correlator_subfile")
...
def test_process_correlator_subfile():
    ...
    # Call to read config <-- this is what we're testing!
    sd.initialise(render_test_config("correlator_subfile"))
    # Override db_handler with a fake one
