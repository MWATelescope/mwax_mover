...
2. [Reference tile selection](#reference-tile-selection)
   ...
   - [NaN channel completeness](#nan-channel-completeness)
...
### NaN channel completeness

At `select_refant` time, `self.jones` has been loaded but no Calvin flagging stages have run. The only NaN channels at this point are from hyperdrive's own solve — channels where the solve did not converge or the data was already NaN in the input visibilities. These are a direct signal about how well the tile calibrated, independent of the phase/gain quality metrics (which are computed only on the surviving non-NaN channels).

For each tile, the NaN count is the number of chanblocks with any NaN Jones element, summed across all solution files in the group. The NaN fraction is this count divided by the total number of chanblocks across all files.
...
Every tile's χ²/dof and σ residual (separately for XX and YY) is then compared against the population of all *other* tiles in the same observation **and of the same receiver type** (rx_type, e.g. RRI/SHAO/NI), using a robust outlier test (see [Median/MAD outlier rejection](#medianmad-outlier-rejection) below). A tile whose XX **or** YY fit is a population outlier on **either** metric is reported as a phase outlier — but, as above, nothing is flagged or changed as a result.
...
**Why scope by receiver type as well as polarisation:** different receiver types have measurably different natural χ²/dof and σ residual distributions, even after each tile's own cable delay has been fit out and removed. On a real observation (`1471082824`, three receiver types — SHAO, RRI, NI), SHAO's population was visibly tighter than RRI's and NI's even excluding genuine outliers (e.g. XX median χ²/dof: SHAO 0.0048, RRI 0.0121, NI 0.0099). Pooling every receiver type into one population before thresholding — as earlier versions of this pipeline did — lets whichever receiver type has the most tiles set a threshold that's too strict for a naturally-noisier minority receiver type (over-reporting it) and too lenient for a naturally-tighter one (under-reporting it). On that same observation, pooling reported 29 tiles, of which roughly 18 turned out to be unremarkable once compared against their own receiver type's population, while 4 tiles from the tighter receiver type that were genuinely anomalous for their own population went undetected, hidden by the noisier receiver types' wider pooled spread. Scoping the threshold per (polarisation, receiver type) instead brought the total down to 15 tiles — the same handful of extreme, unambiguous outliers either way, plus a smaller and more accurate set of borderline cases.
...
- phase_outlier_nstd_threshold: tiles more than this many standard-deviation-equivalents beyond their receiver-type/polarisation population's robust centre are reported as phase outliers (see [Step 6](#step-6-phase-outlier-detection)) -- report-only, does not affect flagging
...
- If the solution files cover fewer coarse channels than the observation's metafits expects (e.g. a gap in a picket-fence observation), the per-coarse-channel gain arrays (`x_gains`, `y_gains`, `x_gains_pol0/1`, `y_gains_sigma_resid`, etc.) are padded with `NaN` at the missing channels' positions (`pad_gain_fit_info` in calibration/fitting.py) so every row has one array entry per metafits coarse channel, in the same order.
...
| `{obs_id}_intercepts.png` | Per receiver-type/polarisation plot of phase intercepts in polar coordinates vs. cable length — another view of the Step 6 delay fit. Rows ordered alphabetically by receiver type, columns XX then YY. |
| `{obs_id}_residual.png` / `_residual.tsv` | Phase residuals vs. frequency, by receiver type and polarisation, with a shaded band showing that group's phase-outlier reporting range (plot and the underlying data as TSV). Same row/column ordering as `intercepts.png`; XX and YY additionally share the same y-axis scale (and tick decimal formatting) within each receiver-type row, so the two are directly comparable -- different receiver-type rows are not forced to share a scale with each other. |
| `{obs_id}_phase_fits.tsv` | All phase-fit statistics (χ²/dof, σ residual, fitted delay, etc.) per tile, as TSV. |
| `{obs_id}_*_solutions_amps.png` / `_solutions_phases.png` | `hyperdrive`'s own plots of calibration solution amplitude/phase vs. fine channel, per tile, using the same [reference tile](#reference-tile-selection) Calvin selected (`--ref-tile`), so they're directly comparable to Calvin's own phase-fit plots below. |
...
| RxType | The tile's receiver type (e.g. RRI/SHAO/NI) — see [Step 6](#step-6-phase-outlier-detection) for why this matters for phase-outlier detection |
...
The population a tile is compared against is itself scoped: a separate threshold is computed and applied independently within each group of rows sharing the same value(s) of one or more grouping columns, rather than pooling every row into one population. Step 6 groups by polarisation and receiver type together, for the reasons given above; Step 4's amplitude-outlier check goes further and doesn't compare tiles against each other at all — see [Step 4](#step-4-amplitude-outlier-flagging).
