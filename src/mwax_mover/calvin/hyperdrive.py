"""Running hyperdrive, and reading/flagging its calibration solution output.
...
coarse-channel band, covering anywhere from 1 to 24 bands for MWA "picket
fence" calibrators -- combined with the observation's metafits): the
single source of truth for reading, flagging, and (eventually) writing
hyperdrive solutions. See calibration/ for the shared data structures and
pure numeric fitting/outlier functions these use (Metafits, fit_phase_line,
fit_gain, annotate_phase_outliers, etc.) and calvin.plots for plotting.
